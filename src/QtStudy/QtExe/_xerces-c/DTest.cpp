...
{
	std::ofstream streamo(strpath.c_str(), std::ios_base::trunc|std::ios_base::binary);
...
int DOMTest::test()
 {
...

     {
		 ...
		 DOMLSOutput       *theOutputDesc = ((DOMImplementationLS*)d->getImplementation())->createLSOutput();

		 StdOutFormatTarget myFormTarget;
		 theOutputDesc->setByteStream(&myFormTarget);
		 
		 //theSerializer->getDomConfig()->setParameter(XMLUni::fgDOMWRTFormatPrettyPrint, true);
		 theSerializer->write(d, theOutputDesc);
		 XMLCh* p = theSerializer->writeToString(d);
