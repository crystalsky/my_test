...
void Color_Dialog::update_widgets()
{
	...
	p->ui.edit_hex->setReadOnly(true);

	p->ui.edit_hex_ARGB->setText(col.name(QColor::HexArgb));
	p->ui.edit_hex_ARGB->setReadOnly(true);

	const QString& strVec4 = 
		QString("osg::Vec4(%1, %2, %3, %4)")
			.arg(col.red() * 1.0 / 255)
			.arg(col.green() * 1.0 / 255)
			.arg(col.blue() * 1.0 / 255)
			.arg(col.alpha() * 1.0 / 255);
	p->ui.edit_Vec4d->setReadOnly(true);
	p->ui.edit_Vec4d->setText(strVec4);
